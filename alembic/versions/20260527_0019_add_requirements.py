...
Introduces Requirements as first-class objects (id, display_id, program_id,
title, description, source_type, status, owner, target_date, link). Extends
the relationships table CHECK constraints to allow 'requirement' as a valid
object type (requires table recreation in SQLite).
"""
...
down_revision: str = "20260527_0018"
branch_labels = None
depends_on = None

_OLD_OBJECT_TYPES = (
    "'work_item', 'dependency', 'risk', 'status_report', 'milestone', 'decision'"
)
_NEW_OBJECT_TYPES = (
...
def upgrade() -> None:
    ...
    op.create_index("ix_requirements_display_id", "requirements", ["display_id"], unique=True)
    op.create_index("ix_requirements_program_id", "requirements", ["program_id"])

    # 2. Expand relationships object-type constraints to include 'requirement'.
...
def downgrade() -> None:
    # Remove requirement-type relationships before tightening constraints.
    op.execute(sa.text(
        "DELETE FROM relationships WHERE source_type = 'requirement' OR target_type = 'requirement'"
    ))
    op.execute(sa.text(f"""
    ...
    op.drop_index("ix_requirements_program_id", table_name="requirements")
    op.drop_index("ix_requirements_display_id", table_name="requirements")
